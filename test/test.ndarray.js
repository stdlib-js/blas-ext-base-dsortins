...
var dsortins = require( './../lib/ndarray.js' );
...
tape( 'the function has an arity of 5', function test( t ) {
	t.strictEqual( dsortins.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns a reference to the input array', function test( t ) {
	...
	out = dsortins( x.length, 1.0, x, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `x` unchanged', function test( t ) {
	...
	dsortins( 0, 1.0, x, 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );

	dsortins( -4, 1.0, x, 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
